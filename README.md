...
- 全部比较只使用未舍入双精度与结构/区间身份，
  **不依赖三位小数展示值，也不按坐标配对**——自交路径上同坐标、不同里程的
  事件位于不同原线段，永远不会被误认为同一事件。

替换区间先做**几何等价**判定：候选替换折线与原被替换折线互为**共线加密**
（简化掉共线中间折点后顶点序列逐点精确一致，整数毫米坐标精确比较，不容差；
折返不算加密）时，两条折线描画同一条物理曲线，物理占用未变：

- **等价（原样替换 / 共线补点）**：两侧碰撞按**连续侵入区间**（同一禁入圈
  在曲线上的极大连通覆盖，不随分段方式变化）分组，每圈各自的区间组沿里程
  一一对应，每组归并为**一个** `remaining` 项，代表点取组内**最近逼近**
  事件——同一段持续重叠不会被拆成多个新增/消除项，数量与位置不随分段
  方式变化，跨越替换边界的连续重叠也不会产生虚假变化提示；
- **非等价（真实绕行）**：保持结构语义——原替换段碰撞整体计入
  `eliminated`，候选替换段碰撞整体计入 `added`。
...
- `circle_risks`：按禁入圈归类的风险变化——
  - `eliminated`：真实绕行时被替换区间段上不再存在的碰撞；
  - `added`：真实绕行时候选替换段上新产生的碰撞；
  - `remaining`：物理占用未变而持续存在的风险——前缀/后缀同一条原线段上
    的碰撞，以及等价替换区间内按侵入区间归并的碰撞（均携带
...
- 测试：pytest（穿越/端点/相切/圈内线段/排序/字段错误/连续侵入区间合并与独立边界/
  ...
  端点衔接 422、标定同源；改线物理占用稳定性：原样替换、共线补点（分段方式
  不变性）、跨替换边界连续重叠、真实绕行四类固定几何样例）、
