...
- 全部比较以**物理占用**（电缆与每个扩张圆在世界坐标中的闭交集点集）
  为身份基准，使用未舍入双精度，**不依赖三位小数展示值，也不按线段下标
  配对**。原样替换（替代折线与原区间逐点重合）不产生任何“消除/新增”；
  共线补点（同一支撑线上插入定位点）只改分段、不改占用，同一段持续
  重叠永远是同一条“仍存在”；连续侵入区间跨越替换边界（占用在边界节点
  两侧不断开）也不产生虚假变化。自交路径同坐标、不同里程的事件是两次
  独立经过，仍分别保留为独立事件，绝不会被误认为同一事件。

  实现：路径节点为整数毫米，线段支撑线用整数典范三元组精确分组，共线
  折片段串成与分段无关的“穿越链（visit）”；在合并后的一维参数上扫描
  配对，零长相切/拐点事件按世界点聚类、把相邻段闭接的 visit 合并为
  “路径经过”后做最小权配对（同线同点优先、拐点跨线次之），再吸收仅为
  正长度片段闭端点的零长项。只有真正离开/进入扩张圈的物理占用才计入
  消除/新增。
...
- `circle_risks`：按禁入圈归类的风险变化（以物理占用点集为身份）——
  - `eliminated`：原线占用、候选线在该物理位置不再占用的连续片段；
  - `added`：候选线新进入扩张圈、原线该物理位置无占用的连续片段；
  - `remaining`：两线在同一物理位置都占用的连续片段（携带
    原线/候选线各自的入口出口点与未舍入里程，差值即里程平移）；
  - 每个片段给出世界坐标 `entry`/`exit`、起止里程与长度，身份按
    “禁入圈 + 世界坐标区间”确定，与被拆成多少段无关；
- 另有 `eliminated_count` / `added_count` / `remaining_count` 汇总。
  原样替换、共线补点、连续侵入跨越替换边界时三项变化计数全为 0
  （仅 `remaining` 反映持续风险）。
...
- 测试：pytest（穿越/端点/相切/圈内线段/排序/字段错误/连续侵入区间合并与独立边界/
  ...
  改线夹具：**原样替换/共线补点/跨替换边界连续重叠零虚假变化、真实绕行
  的消除·新增·持续片段、部分共线分叉、折叠环经过多重集、切点消除与新增、
  跨拐点、前缀沿用与后缀里程平移、自交同坐标异里程、长路径密集细分稳定性、
  端点衔接 422、标定同源）、
